...
install_system_packages() {
    ...
    if [ "$use_sudo" = "true" ]; then
        ...
        micromamba install -c conda-forge rdma-core libibverbs-cos7-x86_64 -y
...
main() {
    ...
    if [ "$USE_SUDO" = "true" ]; then
        ...
        echo "System packages will be installed via mamba (default, safer)"
    fi
    echo ""

    # Install openssl as we overwrite the default version when we update LD_LIBRARY_PATH
    micromamba install -y openssl
